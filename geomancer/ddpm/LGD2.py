...
# from geomancer.model.utils import log_txt_as_img, exists, default, ismap, isimage, mean_flat, count_params, instantiate_from_config
from geomancer.ddpm.ema import LitEma
from geomancer.ddpm.distributions import normal_kl, DiagonalGaussianDistribution
# from ldm.models.autoencoder import VQModelInterface, IdentityFirstStage, AutoencoderKL
from geomancer.model.utils import make_beta_schedule, extract_into_tensor, extract_into_sparse_tensor, noise_like, num2batch, \
    exists, default, ismap, mean_flat, count_params, instantiate_from_config, symmetrize
from geomancer.ddpm.ddim import DDIMSampler
from geomancer.model.Dictionary import *
from geomancer.model.GraphTransformerEncoder import *
from geomancer.model.SyntheticGraphTransformerEncoder import *
from geomancer.model.DenoisingTransformer import DenoisingTransformer
...
class DDPM(pl.LightningModule):
    ...
    def register_schedule(self, given_betas=None, beta_schedule="linear", timesteps=1000,
        ...
        self.register_buffer('posterior_mean_coef4', to_torch(
            (1. - alphas_cumprod_prev) * np.sqrt(alphas)* np.sqrt(alphas_cumprod)/ (1. - alphas_cumprod)))
    ...
    def q_posterior_cfgpp(self, x_c, x_uc, x_t,  t, batch_idx):
        posterior_mean = (
                extract_into_sparse_tensor(self.posterior_mean_coef3, t, batch_idx) * x_uc + 
                extract_into_sparse_tensor(self.posterior_mean_coef2, t, batch_idx) * x_t -
                extract_into_sparse_tensor(self.posterior_mean_coef4, t, batch_idx) * x_c
        )
        posterior_variance = extract_into_sparse_tensor(self.posterior_variance, t, batch_idx)
        posterior_log_variance_clipped = extract_into_sparse_tensor(self.posterior_log_variance_clipped, t, batch_idx)
        return posterior_mean, posterior_variance, posterior_log_variance_clipped
...
class LatentDiffusion_kmeans(DDPM):
    """main class"""
    def __init__(self,
        ...
        if cond_stage_config == '__is_unconditional__':
            conditioning_key = None
        elif cond_stage_config == '__is_unconditional__kmeans':
        ...
        self.guidance_scale = cfg.diffusion.guidance_scale
        self.guidance_type = cfg.diffusion.get("guidance_type", 'cfg')
    ...
    def instantiate_cond_stage(self, config):
        ...
        elif config == "__is_unconditional__" or config == '__is_unconditional__kmeans':
    ...
    def get_input(self, batch, prompt_graph_batch=None, return_first_stage_outputs=False, force_c_encode=False,
        ...
        if self.model.conditioning_key is not None:
            ...
            if cond_key == 'masked_graph':
                ...
                if not self.cond_stage_trainable:
                    c = c.detach()
            elif cond_key == 'kmeans':
                # xc = (batch.kmeans_x.detach(),batch.kmeans_edge.detach())
                xc = batch.cond_y.detach()
                c = xc
                if len(c.shape) == 1:
                    c = c.unsqueeze(1)
                # if len(c.shape) == 2:
    ...
    def get_condition_output(self,batch_noisy,t,cond):
        batch_noisy2 = batch_noisy.clone()
        batch_output = self.model(batch_noisy, t, None)
        batch_output_cond = self.model(batch_noisy2, t, cond)
        batch_output.x =batch_output_cond.x*self.guidance_scale +  batch_output.x *(1-self.guidance_scale)
        batch_output.edge_attr = batch_output_cond.edge_attr*self.guidance_scale + batch_output.edge_attr *(1-self.guidance_scale)
        return batch_output
    
    def get_condition_outputpp(self,batch_noisy,t,cond):
        batch_noisy2 = batch_noisy.clone()
        batch_output_uncond = self.model(batch_noisy, t, None)
        batch_output_cond = self.model(batch_noisy2, t, cond)
        batch_output_cond.x =batch_output_cond.x*self.guidance_scale +  batch_output_uncond.x *(1-self.guidance_scale)
        batch_output_cond.edge_attr = batch_output_cond.edge_attr*self.guidance_scale + batch_output_uncond.edge_attr *(1-self.guidance_scale)
        return batch_output_cond,batch_output_uncond
    
    def get_condition_output2(self,batch_noisy,t,cond, drop_out=0.9):
    ...
    def p_losses(self, batch, cond, t, batch_idx, noise=None):
        ...
        if self.cond_stage_config == '__is_unconditional__kmeans':
            batch_output = self.get_condition_output2(batch_noisy,t,cond,drop_out=1-self.guidance_scale)
        else: 
            batch_output = self.model(batch_noisy, t, cond)
        ...
        if cfg.train.mode in ['qm9_unconditional', 'qm9_conditional', 'qm9_kmeans']:
            generated_mol = []
    ...
    def p_mean_variance(self, batch_x, c, t, batch_idx, clip_denoised: bool, return_codebook_ids=False, quantize_denoised=False,
        ...
        if self.cond_stage_config == '__is_unconditional__kmeans':
            if self.guidance_type =='cfg':
                batch_model_out = self.get_condition_output(batch_x,t_in,c)
            else:
                batch_model_out_c, batch_model_out_uc = self.get_condition_outputpp(batch_x,t_in,c)
        else: 
            batch_model_out = self.model(batch_x, t_in, c)
        
        if score_corrector is not None:
            assert self.parameterization == "eps"
            if self.guidance_type == 'cfgpp':
                batch_model_out_uc = score_corrector.modify_score(self, batch_model_out_uc, batch_x, t, None, **corrector_kwargs)
                batch_model_out_c = score_corrector.modify_score(self, batch_model_out_c, batch_x, t, c, **corrector_kwargs)
            else:
        ...
        if self.guidance_type =='cfgpp':
            if self.parameterization == "x0":
                x_recon_c = torch.cat([batch_model_out_c.x, batch_model_out_c.edge_attr], dim=0)
                x_recon_uc = torch.cat([batch_model_out_uc.x, batch_model_out_uc.edge_attr], dim=0)
            else:
                raise NotImplementedError()
            if clip_denoised:
                x_recon_c.clamp_(-1., 1.)
                x_recon_uc.clamp_(-1., 1.)
            if quantize_denoised:
                x_recon_c, _, [_, _, indices] = self.first_stage_model.quantize(x_recon_c)
                x_recon_uc, _, [_, _, indices] = self.first_stage_model.quantize(x_recon_uc)
            model_mean, posterior_variance, posterior_log_variance = self.q_posterior_cfgpp(x_c=x_recon_c, x_uc=x_recon_uc, x_t=x, t=t, batch_idx=batch_idx)
        else:
            if self.parameterization == "eps":
    ...
    def p_sample_loop(self, batch, cond, shape, batch_idx, return_intermediates=False,
        ...
        for i in iterator:
            ...
            #     batch.x = img[: batch.num_nodes]
            #     batch.edge_attr = img[batch.num_nodes:]
            img = self.decode_first_stage(img)
            if i % log_every_t == 0 or i == timesteps - 1:
                intermediates.append(img)
            if callback: callback(i)
            if img_callback: img_callback(img, i)
