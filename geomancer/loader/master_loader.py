...
from geomancer.transform.posenc_stats import compute_posenc_stats
from geomancer.transform.transforms import (pre_transform_in_memory, generate_splits,
                                           typecast_x, concat_x_and_pos,
                                           clip_graphs_to_size, move_node_feat_to_x)
from geomancer.transform.expander_edges import generate_random_expander
from geomancer.transform.dist_transforms import (add_dist_features, add_reverse_edges,
                                                 add_self_loops, effective_resistances,
                                                 effective_resistance_embedding,
                                                 effective_resistances_from_embedding,my_remove_loops,my_toundirected)
from geomancer.transform.transforms import (add_virtual_node_edge, no_virtual_node_edge, pretransform_QM9_generation,\
    ...
    if format.startswith('PyG-'):
        ...
        #     dataset = Planetoid(dataset_dir, name)
            ...
            if cfg.dataset.get('split_mode', 'random') == 'standard':
                ...
                dataset = Planetoid_Custom(dataset_dir, name, split='random_perclass', train_percent=cfg.prep.train_percent)
            ...
            if cfg.prep.add_reverse_edges == True:
                pre_transform_in_memory(dataset, partial(my_toundirected))
            if cfg.prep.add_self_loops == True:
                # pre_transform_in_memory(dataset, partial(my_remove_loops))
                pre_transform_in_memory(dataset, partial(add_self_loops))
            # if cfg.prep.add_reverse_edges == True:
            #     pre_transform_in_memory(dataset, partial(add_reverse_edges))
            # if cfg.prep.add_self_loops == True:
...
def preformat_OGB_Graph(dataset_dir, name):
    ...
    if name == 'ogbg-ppa':
        ...
        from geomancer.loader.ogbg_code2_utils import idx2vocab, \
...
def preformat_PCQM4Mv2Contact(dataset_dir, name):
    ...
    try:
        # Load locally to avoid RDKit dependency until necessary
        from geomancer.loader.dataset.pcqm4mv2_contact import \
...
def preformat_Peptides(dataset_dir, name):
    ...
    try:
        # Load locally to avoid RDKit dependency until necessary.
        from geomancer.loader.dataset.peptides_functional import \
            PeptidesFunctionalDataset
        from geomancer.loader.dataset.peptides_structural import \
...
def preformat_Spectre(dataset_dir, name):
    from geomancer.loader.dataset.synthetic_dataset import SpectreGraphDataset
...
def preformat_MOSES(dataset_dir, name):
    from geomancer.loader.dataset.moses import MOSESDataset
...
def preformat_QM9(dataset_dir, name):
    ...
    if name == 'standard':
        ...
        from geomancer.loader.dataset.qm9 import QM9Dataset
