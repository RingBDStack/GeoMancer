...
def pretrain_epoch(logger, loader, model, optimizer, r_optim, scheduler, batch_accumulation, r_grad=False, visualize=False):
    model.train()
    optimizer.zero_grad()
    r_optim.zero_grad()
...
@register_train('pretrain_encoder_rm')
def custom_pretrain_encoder(loggers, loaders, model, optimizer, scheduler, r_optim):
    """
    ...
    for cur_epoch in range(start_epoch, cfg.optim.max_epoch):
        if cur_epoch%cfg.optim.interval_epoch==0:
            r_grad=True
        else:
            r_grad=False
