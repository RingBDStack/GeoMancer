...
def train_epoch(logger, loader, model, optimizer, scheduler, batch_accumulation, train_smiles, dataset_info, labels):
    model.train()
    optimizer.zero_grad()
    # center_list = torch.load('center_0.pt')
    ...
    for iter, batch in enumerate(loader):
        ...
        batch_size = batch.y.size(0)  # 假设每个batch的size是batch.graph_attr的第一个维度
        batch.cond_y = torch.tensor(labels[label_idx:label_idx + batch_size]).to(batch.y.device)
...
def eval_epoch(logger, loader, model, split='val', repeat=1, ensemble_mode='none', train_smiles=None, dataset_info=None, evaluate=False, pref=None, current_epoch=0):
    ...
    num_test_graphs = 0
    # if valid:
    #     center_list = torch.load('center_1.pt')
    # else:
    #     center_list = torch.load('center_2.pt')
    for batch in loader:
        max_num_test = 1000 if cfg.dataset.format == 'PyG-MOSES' else 10000
        print('num_test_graphs')
        print(num_test_graphs)
        ...
        if cfg.gnn.head == 'inductive_edge':
            ...
            if ensemble_mode == 'none':
                ...
                batch.x_masked = batch.x.clone().detach()
                batch.edge_attr_masked = batch.edge_attr.clone().detach()
                batch.cond_y = torch.randint(0, cfg.diffusion.cluster, (batch.y.size(0),)).to(torch.device(cfg.accelerator))
...
@torch.no_grad()
def eval_epoch2(logger, loader, model, split='val', repeat=1, ensemble_mode='none', train_smiles=None, dataset_info=None, evaluate=False, pref=None, current_epoch=0):
    model.eval()
    time_start = time.time()
    generated_mol = []
    iter = 0
    ...
    if evaluate:
        ...
        for i in range(len(visualize_samples)):
            ...
            plt.savefig(save_path + '/epoch_' + str(current_epoch) + '_sample_' + str(i) + '.png')
            plt.clf()
@register_train('qm9_kmeans2')
def custom_train_diffusion(loggers, loaders, model, optimizer, scheduler,labels_pre):
    """
    ...
    for cur_epoch in range(start_epoch, cfg.optim.max_epoch):
        ...
        train_epoch(loggers[0], loaders[0], model, optimizer, scheduler,
                    cfg.optim.batch_accumulation, train_smiles, infos, labels_pre)
        ...
        if cur_epoch > cfg.train.start_eval_epoch:
            ...
            if (is_ckpt_epoch(cur_epoch) or ((cur_epoch + 1) % 100 == 0)):
...
@register_train('qm9_kmeans')
def custom_train_diffusion_debug(loggers, loaders, model, optimizer, scheduler,labels_pre):
    """
    Customized training pipeline.
    ...
    cur_epoch=0
    eval_epoch2(loggers[0], loaders[1], model,
                               split=split_names[0], repeat=cfg.train.ensemble_repeat,
                               ensemble_mode=cfg.train.ensemble_mode,
                               train_smiles=train_smiles, dataset_info=infos, evaluate=True, pref=pref, current_epoch=cur_epoch)
