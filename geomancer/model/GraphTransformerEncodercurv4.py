...
@register_layer("GTE_layer_curv4")
...
@register_network('GraphTransformerEncoder_curv4')
class GraphTransformerEncoder_curv4(nn.Module):
    """
    ...
    def __init__(self, dim_in=0, dim_out=0,  # in_dim, posenc_in_dim, posenc_dim, prefix_dim, hid_dim, out_dim,
        ...
        if self.rm_encode:
            ...
            for i in range(num_factors):
                ...
                self.bias_encode.append(2 * torch.pi * torch.rand(len(self.curv1),d))
                self.lambda_encode.append(torch.rand(len(self.curv1),d))

            
       
            self.encode_kernel_layer = nn.Linear((num_factors+1)*self.hid_dim,self.hid_dim)
            if cfg.encode_layer_norm:
                self.encode_kernel_norm = nn.LayerNorm(self.hid_dim)
            if cfg.encode_batch_norm:
                self.encode_kernel_norm = nn.BatchNorm1d(self.hid_dim, track_running_stats=not self.bn_no_runner, eps=1e-5,
                                                  momentum=self.bn_momentum)
        ...
        if self.rm_decode:
            ...
            #construct kernel
            self.rm_decode_feature = nn.ParameterList()
            for i,k in enumerate(self.curv2):
                features = ManifoldParameter(ManifoldTensor(torch.empty(self.out_dim, self.out_dim), manifold=self.rm_decode_manifolds[i]))
                self.init_weights(features)
                self.rm_decode_feature.append(project(features, k=manifold.k))
            self.Ws_decode = nn.ParameterList()
            self.bias_decode = nn.ParameterList()
            self.lambda_decode = nn.ParameterList()
            for i in range(num_factors):
                d = self.out_dim
                pre = torch.randn(len(self.curv2), d, d)
                w = pre / (torch.norm(pre, dim=-1, keepdim=True) + EPS)
                self.Ws_decode.append(w)
                self.bias_decode.append(2 * torch.pi * torch.rand(len(self.curv2),d))
                self.lambda_decode.append(torch.rand(len(self.curv2),d))
            
            self.decode_kernel_layer = nn.Linear((num_factors+1)*self.out_dim,self.out_dim)
            if cfg.decode_layer_norm:
                self.decode_kernel_norm = nn.LayerNorm(self.out_dim)
            if cfg.encode_batch_norm:
                self.deode_kernel_norm = nn.BatchNorm1d(self.out_dim, track_running_stats=not self.bn_no_runner, eps=1e-5,
                                                  momentum=self.bn_momentum)
    ...
    def random_mapping_multi(self,manifolds,factor_number,products,Ws,bias):
        # print('success')
        out = []
        rm_list = products
    ...
    def decode_node_curv(self,node_attr):
        embeds = self.rm_decode_feature.random_mapping_multi()
        emb_list = []
        for emb in embeds:
            tmp_x = node_attr
            tmp_x = tmp_x@emb
...
# @register_network('GraphTransformerStructureEncoder_curv4')
# class GraphTransformerStructureEncoder_curv4(nn.Module):
#     """
