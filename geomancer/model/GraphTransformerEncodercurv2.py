...
@register_network('GraphTransformerEncoder_curv2')
class GraphTransformerEncoder(nn.Module):
    ...
    def __init__(self, dim_in=0, dim_out=0,  # in_dim, posenc_in_dim, posenc_dim, prefix_dim, hid_dim, out_dim,
                 ...
                 **kwargs):
        super().__init__()
        # print("GraphTransformerEncoder registered")
        # if cfg is None:
        #     cfg = dict()
        # logging.info(cfg)
        # cfg = cfg.encoder
        ...
        if self.label_embed_type not in ['add_virtual', 'add_all']:
            self.label_embed_type = 'add_virtual'
        ...
        self.rm_encode = cfg.rm_encoder
        self.rm_decode = cfg.rm_decoder
        ...
        if self.rm_encode:
            ...
            #构建node_kernel
            self.rm_encode_node_feature = nn.ParameterList()
            for i,k in enumerate(self.curv1):
                features = ManifoldParameter(ManifoldTensor(torch.empty(self.hid_dim, self.hid_dim), manifold=self.rm_encode_manifolds[i]))
                self.init_weights(features)
                self.rm_encode_node_feature.append(project(features, k=manifold.k))
            self.Ws_encode_node = nn.ParameterList()
            self.bias_encode_node = nn.ParameterList()
            self.lambda_encode_node = nn.ParameterList()
            for i in range(num_factors):
                d = self.hid_dim
                pre = torch.randn(len(self.curv1), d, d)
                w = pre / (torch.norm(pre, dim=-1, keepdim=True) + EPS)
                self.Ws_encode_node.append(w)
                self.bias_encode_node.append(2 * torch.pi * torch.rand(len(self.curv1),d))
                self.lambda_encode_node.append(torch.rand(len(self.curv1),d))

            #构建edge_kernel
            self.rm_encode_edge_feature = nn.ParameterList()
            for i,k in enumerate(self.curv1):
                features = ManifoldParameter(ManifoldTensor(torch.empty(self.hid_dim, self.hid_dim), manifold=self.rm_encode_manifolds[i]))
                self.init_weights(features)
                self.rm_encode_edge_feature.append(project(features, k=manifold.k))
            self.Ws_encode_edge = nn.ParameterList()
            self.bias_encode_edge = nn.ParameterList()
            self.lambda_encode_edge = nn.ParameterList()
            for i in range(num_factors):
                d = self.hid_dim
                pre = torch.randn(len(self.curv1), d, d)
                w = pre / (torch.norm(pre, dim=-1, keepdim=True) + EPS)
                self.Ws_encode_edge.append(w)
                self.bias_encode_edge.append(2 * torch.pi * torch.rand(len(self.curv1),d))
                self.lambda_encode_node.append(torch.rand(len(self.curv1),d))

            self.encode_kernellayer_edge = nn.Linear((num_factors+1)*self.hid_dim,self.hid_dim)
            # self.rm_encode_edge_norm = nn.LayerNorm(self.hid_dim)
            self.encode_kernellayer_edge_norm = nn.BatchNorm1d(self.hid_dim, track_running_stats=not self.bn_no_runner, eps=1e-5,
                                                  momentum=self.bn_momentum)
            self.rm_encode_kernellayer_node = nn.Linear((num_factors+1)*self.hid_dim,self.hid_dim)
            # self.rm_encode_node_norm = nn.LayerNorm(self.hid_dim)
            self.rm_encode_kernellayer_node_norm = nn.BatchNorm1d(self.hid_dim, track_running_stats=not self.bn_no_runner, eps=1e-5,
                                                  momentum=self.bn_momentum)
        
        if self.rm_decode:
            self.curv2 = cfg.decode_curv
            self.rm_decode_manifolds = nn.ModuleList()
            for i,k in enumerate(self.curv2):
                manifold = StereographicExact(k=k, learnable=learnable)
                self.rm_decode_manifolds.append(manifold)
            self.decode_num_factors = cfg.decode_factor
            num_factors= self.decode_num_factors

            #构建node_kernel
            self.rm_decode_node_feature = nn.ParameterList()
            for i,k in enumerate(self.curv2):
                features = ManifoldParameter(ManifoldTensor(torch.empty(self.out_dim, self.out_dim), manifold=self.rm_decode_manifolds[i]))
                self.init_weights(features)
                self.rm_decode_node_feature.append(project(features, k=manifold.k))
            self.Ws_decode_node = nn.ParameterList()
            self.bias_decode_node = nn.ParameterList()
            self.lambda_decode_node = nn.ParameterList()
            for i in range(num_factors):
                d = self.out_dim
                pre = torch.randn(len(self.curv2), d, d)
                w = pre / (torch.norm(pre, dim=-1, keepdim=True) + EPS)
                self.Ws_decode_node.append(w)
                self.bias_decode_node.append(2 * torch.pi * torch.rand(len(self.curv2),d))
                self.lambda_decode_node.append(torch.rand(len(self.curv2),d))
                
            #构建edge_kernel
            self.rm_decode_edge_feature = nn.ParameterList()
            for i,k in enumerate(self.curv2):
                features = ManifoldParameter(ManifoldTensor(torch.empty(self.out_dim, self.out_dim), manifold=self.rm_decode_manifolds[i]))
                self.init_weights(features)
                self.rm_decode_edge_feature.append(project(features, k=manifold.k))
            self.Ws_decode_edge = nn.ParameterList()
            self.bias_decode_edge = nn.ParameterList()
            self.lambda_decode_edge = nn.ParameterList()
            for i in range(num_factors):
                d = self.out_dim
                pre = torch.randn(len(self.curv2), d, d)
                w = pre / (torch.norm(pre, dim=-1, keepdim=True) + EPS)
                self.Ws_decode_edge.append(w)
                self.bias_decode_edge.append(2 * torch.pi * torch.rand(len(self.curv2),d))
                self.lambda_decode_edge.append(torch.rand(len(self.curv2),d))
            #构建graph_kernel
            self.rm_decode_graph_feature = nn.ParameterList()
            for i,k in enumerate(self.curv2):
                features = ManifoldParameter(ManifoldTensor(torch.empty(self.hid_dim, self.hid_dim), manifold=self.rm_decode_manifolds[i]))
                self.init_weights(features)
                self.rm_decode_graph_feature.append(project(features, k=manifold.k))
            self.Ws_decode_edge = nn.ParameterList()
            self.bias_decode_edge = nn.ParameterList()
            self.lambda_decode_graph.append(torch.rand(len(self.curv2),d))
            for i in range(num_factors):
                d = self.hid_dim
                pre = torch.randn(len(self.curv2), d, d)
                w = pre / (torch.norm(pre, dim=-1, keepdim=True) + EPS)
    ...
    def normalize(self, x, manifold):
        ...
        if manifold.k != 0:
            x = x / x_norm * 0.9 * torch.rand(1) * manifold.radius
        else:
            x = x / x_norm
        return x
    
    def random_mapping_multi(self,manifolds,products,Ws,bias):
        # print('success')
        out = []
        rm_list = products
    ...
    def encode_edge_curv(self,edge_attr):
        embeds = self.rm_encode_node_feature.random_mapping_multi()
        emb_list = []
        for emb in embeds:
            tmp_x = edge_attr
            tmp_x = tmp_x@emb
            tmp_x = self.rm_encode_edge_norm(tmp_x)
            emb_list.append(tmp_x)
        emb_list.append(edge_attr)
        embeds_tensor = torch.concat(emb_list,-1)
        
      
        embeds_tensor = self.rm_encode_layer_edge(embeds_tensor)
    ...
    def encode_node_curv(self,node_attr):
        ...
        for emb in embeds:
            tmp_x = node_attr
            tmp_x = tmp_x@emb
            tmp_x = self.rm_encode_node_norm(tmp_x)
            emb_list.append(tmp_x)
        emb_list.append(node_attr)
        embeds_tensor = torch.concat(emb_list,-1)
        
      
        embeds_tensor = self.rm_encode_layer_node(embeds_tensor)

        return embeds_tensor
    
    def decode_edge_curv(self,edge_attr):
        embeds = self.rm_decode_edge_feature.random_mapping_multi()
        emb_list = []
        for emb in embeds:
            tmp_x = edge_attr
            tmp_x = tmp_x@emb
            tmp_x = self.rm_decode_edge_norm(tmp_x)
            emb_list.append(tmp_x)
        emb_list.append(edge_attr)
        embeds_tensor = torch.concat(emb_list,-1)
        embeds_tensor = self.rm_decode_layer_edge(embeds_tensor)
        return embeds_tensor

    def decode_node_curv(self,node_attr):
        embeds = self.rm_decode_node_feature.random_mapping_multi()
        emb_list = []
        for emb in embeds:
            tmp_x = node_attr
            tmp_x = tmp_x@emb
            tmp_x = self.rm_decode_node_norm(tmp_x)
            emb_list.append(tmp_x)
        emb_list.append(node_attr)
        embeds_tensor = torch.concat(emb_list,-1)
        embeds_tensor = self.rm_decode_layer_node(embeds_tensor)
        return embeds_tensor

    def decode_graph_curv(self, graph_attr):
        embeds = self.rm_decode_graph_feature.random_mapping_multi()
        emb_list = []
        for emb in embeds:
            tmp_x = graph_attr
            tmp_x = tmp_x@emb
            tmp_x = self.rm_decode_graph_norm(tmp_x)
            emb_list.append(tmp_x)
        emb_list.append(graph_attr)
        embeds_tensor = torch.concat(emb_list,-1)
        embeds_tensor = self.rm_decode_layer_graph(embeds_tensor)
        return embeds_tensor
    ...
    def forward(self, batch, t=None, prefix=None, label=None, **kwargs):
        ...
        if self.rm_encode:
            h = self.encode_node_curv(h)
            e = self.encode_edge_curv(e)
    ...
    def decode(self, batch, **kwargs):
        if self.rm_decode:
            batch.x = self.decode_node_curv(batch.x)
            batch.edge_attr = self.decode_edge_curv(batch.edge_attr)
            batch.graph_attr = self.decode_graph_curv(batch.graph_attr)
...
@register_network('GraphTransformerDecoder_curv2')
...
@register_network('GraphTransformerStructureEncoder_curv2')
class GraphTransformerStructureEncoder_curv2(nn.Module):
    """
    ...
    def __init__(self, dim_in=0, dim_out=0,  # in_dim, posenc_in_dim, posenc_dim, prefix_dim, hid_dim, out_dim,
        ...
        self.Ws = []
        self.bias = []
        num_factors = len(self.curv)
        num_factors2 = num_factors+1
        self.decode_node = nn.Linear(self.out_dim, self.node_dict_dim)  # virtual node
        self.decode_edge = nn.Linear(self.out_dim, self.edge_dict_dim)  # no edge, virtual edge, loop
        self.decode_graph = nn.Linear(self.out_dim, self.decode_dim)
        ...
        self.rm_norm = nn.LayerNorm(self.out_dim)
        self.rm_layer_node = nn.Linear(self.out_dim*num_factors2,self.out_dim, bias=False)
        self.rm_layer_edge = nn.Linear(self.out_dim*num_factors2,self.out_dim, bias=False)
        self.rm_layer_graph = nn.Linear(self.out_dim*num_factors2,self.out_dim, bias=False)
        self.rm_layer_node_from_edge = nn.Linear(self.out_dim*num_factors2,self.out_dim, bias=False)
        self.rm_layer_edge_from_node = nn.Linear(self.out_dim*num_factors2,self.out_dim, bias=False)
        self.rm_layer_nodes_from_edge = nn.Linear(self.out_dim*num_factors2,self.out_dim, bias=False)
        self.rm_layer_pe_from_node = nn.Linear(self.out_dim*num_factors2,self.out_dim, bias=False)
        self.rm_layer_pe_from_edge = nn.Linear(self.out_dim*num_factors2,self.out_dim, bias=False)
        self.decode_node2 = nn.Linear(self.out_dim, self.node_dict_dim)  # virtual node
        self.decode_edge2 = nn.Linear(self.out_dim, self.edge_dict_dim)  # no edge, virtual edge, loop
        self.decode_graph2 = nn.Linear(self.out_dim, self.decode_dim)
        self.decode_node_from_edge2 = nn.Linear(self.out_dim, self.node_dict_dim)  # virtual node
        self.decode_edge_from_node2 = nn.Linear(self.out_dim, self.edge_dict_dim)
        self.decode_nodes_from_edge2 = nn.Linear(self.out_dim, int(self.node_dict_dim * (self.node_dict_dim + 1) / 2))
        
       
        dimensions = cfg.out_dim
        device = 'cuda'
        self.device = device
        
        self.rm_layer_node_feature = RiemannianFeatures(self.out_dim,self.out_dim,self.curv,num_factors)
        self.rm_layer_edge_feature = RiemannianFeatures(self.out_dim,self.out_dim,self.curv,num_factors)
        self.rm_layer_graph_feature = RiemannianFeatures(self.out_dim,self.out_dim,self.curv,num_factors)
        self.rm_layer_node_from_edge_feature = RiemannianFeatures(self.out_dim,self.out_dim,self.curv,num_factors)
        self.rm_layer_edge_from_node_feature = RiemannianFeatures(self.out_dim,self.out_dim,self.curv,num_factors)
        self.rm_layer_nodes_from_edge_feature = RiemannianFeatures(self.out_dim,self.out_dim,self.curv,num_factors)
        # self.rm_layer_pe_from_node_feature = RiemannianFeatures(self.out_dim,self.out_dim,self.curv,num_factors)
        # self.rm_layer_pe_from_edge_feature = RiemannianFeatures(self.out_dim,self.out_dim,self.curv,num_factors)
        
    def init_rm_feature(self):
        rm_feature_list = []
        for init_curvature  in self.curv:
            k = torch.tensor(init_curvature)
            manifold = StereographicExact(k=k, learnable=False)
            self.manifold_list.append(manifold)
            rm_feature = ManifoldParameter(ManifoldTensor(torch.empty(cfg.out_dim, cfg.out_dim), manifold=manifold))
            if k != 0:
                self.init_weights(rm_feature)
                rm_feature.data = project(rm_feature, k=k)
            else:
                rm_feature.data = torch.randn(rm_feature.shape)
            rm_feature = rm_feature.to(self.device)
            rm_feature_list.append(rm_feature)
        return rm_feature_list
    ...
    def encode_edge_curv(self,edge_attr):
        embeds = self.rm_layer_edge_feature.random_mapping_multi()
        emb_list = []
        for emb in embeds:
            tmp_x = edge_attr
            tmp_x = tmp_x@emb
            tmp_x = self.rm_norm(tmp_x)
            tmp_x = torch.clamp(tmp_x, min=epsilon)
            emb_list.append(tmp_x)
        emb_list.append(edge_attr)
        embeds_tensor = torch.concat(emb_list,-1)
        ...
        # embeds_tensor = self.rm_norm(emb_list)
        embeds_tensor = self.rm_layer_edge(embeds_tensor)

        edge_tmp = self.decode_edge2(embeds_tensor)
        return edge_tmp
    def forward(self, batch, t=None, prefix=None, label=None, **kwargs):
        # batch.x_0 = batch.x
        # batch.edge_attr_0 = batch.edge_attr
    ...
    def random_mapping_shallow(self, manifolds, x):
        out = []
        w=self.Ws[0]
        b=self.bias[0]
        k=manifolds.k
    ...
    # def random_mapping_multi(self, rm_list):
    #     out = []
        
    #     for i in range(len(self.manifold_list)):
    #         manifold = self.manifold_list[i]
    #         x = rm_list[i]
    #         assert not torch.isnan(x).any(), "Tensor x contains NaN values"
    #         w = self.Ws[i]
    #         b = self.bias[i]
    ...
    def encode(self, batch, t=None, prefix=None, label=None, **kwargs):
        return self.forward(batch, t, prefix, label, **kwargs)
    def decode_node_curv(self,x):
        embeds = self.rm_layer_node_feature.random_mapping_multi()
        ...
        for emb in embeds:
            ...
            assert not torch.isnan(tmp_x).any(), "Tensor tmp_x contains NaN values"
            torch.save(tmp_x,'tmp_x.pt')
            tmp_x = self.rm_norm(tmp_x)
            tmp_x = torch.clamp(tmp_x, min=epsilon)
            torch.save(emb,'emb.pt')
            torch.save(tmp_x,'after_norm.pt')
            assert not torch.isnan(tmp_x).any(), "Tensor norm_tmp contains NaN values"
        ...
        # print(emb_list.shape)
        # torch.save(embeds,'embeds.pt')
        # torch.save(embeds_tensor,'embeds_tensor.pt')
       
        if debug2:
            print('node')
            print(embeds_tensor)
        embeds_tensor = self.rm_layer_node(embeds_tensor)
        assert not torch.isnan(embeds_tensor).any(), "Tensor embeds_tensor contains NaN values"
        x_tmp = self.decode_node2(embeds_tensor)
        assert not torch.isnan(x_tmp).any(), "Tensor x_tmp contains NaN values"
        return x_tmp
    def decode_edge_curv(self,edge_attr):
        embeds = self.rm_layer_edge_feature.random_mapping_multi()
        emb_list = []
    ...
    def decode_graph_curv(self,graph_attr):
        embeds = self.rm_layer_graph_feature.random_mapping_multi()
        ...
        if debug2:
            ...
            print(emb_list)
        # embeds_tensor = self.rm_norm(emb_list)
        embeds_tensor = self.rm_layer_graph(embeds_tensor)

        graph_tmp = self.decode_graph2(embeds_tensor)
        return graph_tmp

    def decode_node_from_edge_curv(self,graph_attr):
        embeds = self.rm_layer_node_from_edge_feature.random_mapping_multi()
        emb_list = []
        for emb in embeds:
            tmp_x = graph_attr
            tmp_x = tmp_x@emb
        ...
        if debug:
            ...
            print(emb_list)
        # embeds_tensor = self.rm_norm(emb_list)
        embeds_tensor = self.rm_layer_node_from_edge(embeds_tensor)

        graph_tmp = self.decode_node_from_edge2(embeds_tensor)
        return graph_tmp

    def decode_edge_from_node_curv(self,graph_attr):
        embeds = self.rm_layer_edge_from_node_feature.random_mapping_multi()
        emb_list = []
        for emb in embeds:
            tmp_x = graph_attr
            tmp_x = tmp_x@emb
        ...
        # print(emb_list.shape)
        # embeds_tensor = self.rm_norm(emb_list)
        embeds_tensor = self.rm_layer_edge_from_node(embeds_tensor)

        graph_tmp = self.decode_edge_from_node2(embeds_tensor)
        return graph_tmp

    def decode_nodes_from_edge_curv(self,graph_attr):
        embeds = self.rm_layer_nodes_from_edge_feature.random_mapping_multi()
        emb_list = []
        for emb in embeds:
            tmp_x = graph_attr
            tmp_x = tmp_x@emb
        ...
        # print(emb_list.shape)
        # embeds_tensor = self.rm_norm(emb_list)
        embeds_tensor = self.rm_layer_nodes_from_edge(embeds_tensor)

        graph_tmp = self.decode_nodes_from_edge2(embeds_tensor)
        return graph_tmp
    def decode_pe_from_node_curv(self,graph_attr,embeds):
        emb_list = []
        for emb in embeds:
            tmp_x = graph_attr
            tmp_x = tmp_x@emb
        ...
        # print(emb_list.shape)
        # embeds_tensor = self.rm_norm(emb_list)
        embeds_tensor = self.rm_layer_pe_from_node(embeds_tensor)

        graph_tmp = self.decode_pe_from_node(graph_attr)
        return graph_tmp

    def decode_pe_from_edge_curv(self,graph_attr,embeds):
        emb_list = []
        for emb in embeds:
            tmp_x = graph_attr
            tmp_x = tmp_x@emb
        ...
        # print(emb_list.shape)
        # embeds_tensor = self.rm_norm(emb_list)
        embeds_tensor = self.rm_layer_pe_from_edge(embeds_tensor)

        graph_tmp = self.decode_pe_from_edge(graph_attr)
        return graph_tmp

    def decode(self, batch, **kwargs):
        # embeds = self.random_mapping_multi(self.rm_feature_list)
        
        return self.decode_node_curv(batch.x), self.decode_edge_curv(batch.edge_attr), self.decode_graph_curv(batch.graph_attr).flatten()
    ...
    def decode_recon(self, batch, **kwargs):
        ...
        return self.decode_node_from_edge2(score_node), self.decode_edge_from_node2(score_edge), self.decode_nodes_from_edge2(batch.edge_attr), \
