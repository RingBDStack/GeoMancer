...
import warnings
from geomancer.model.GraphTransformerEncoder import GTE_Attention, GraphTransformerEncoderLayer
from geomancer.model.utils import pyg_softmax, num2batch, get_timestep_embedding, get_log_deg
...
class Cross_Attention(nn.Module):
    ...
    def __init__(self, in_dim, out_dim, condition, num_heads, use_bias,
        ...
        assert condition in ['masked_graph', 'prompt_graph', 'prefix', 'class_label', 'multi-modal', 'unconditional','kmeans']
    ...
    def forward(self, batch, prompt=None):
        ...
        if self.condition == 'masked_graph':  # TODO: consider prompt_g
            ...
            # print('this is cross-attention')
            # print(prompt.shape)
            # print(prompt_e.shape)
            # print(prompt_g.shape)
...
class DenoisingTransformer(nn.Module):
    ...
    #              cfg=dict(),
        ...
        self.cond_emb = nn.Embedding(cfg.diffusion.cluster, self.cond_dim)
    ...
    def forward(self, batch, t=None, prompt=None, **kwargs):
        ...
        if 'masked_graph' in self.condition_list:  # TODO: prompt_graph, how to process? the hidden_dim are different too
            ...
            prompt = None
        elif 'kmeans' in self.condition_list:
          
            prompt = self.cond_emb(prompt)
