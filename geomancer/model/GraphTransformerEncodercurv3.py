...
@register_network('GraphTransformerEncoder_curv3')
...
@register_network('GraphTransformerDecoder_curv3')
...
@register_network('GraphTransformerStructureEncoder_curv3')
class GraphTransformerStructureEncoder_curv3(nn.Module):
    """
    ...
    def __init__(self, dim_in=0, dim_out=0,  # in_dim, posenc_in_dim, posenc_dim, prefix_dim, hid_dim, out_dim,
        ...
        self.rm_layer_node = nn.Linear(self.out_dim*3,self.out_dim, bias=False)
        self.rm_layer_edge = nn.Linear(self.out_dim*3,self.out_dim, bias=False)
        self.rm_layer_graph = nn.Linear(self.out_dim*3,self.out_dim, bias=False)
        self.rm_layer_node_from_edge = nn.Linear(self.out_dim*3,self.out_dim, bias=False)
        self.rm_layer_edge_from_node = nn.Linear(self.out_dim*3,self.out_dim, bias=False)
        self.rm_layer_nodes_from_edge = nn.Linear(self.out_dim*3,self.out_dim, bias=False)
        self.rm_layer_pe_from_node = nn.Linear(self.out_dim*3,self.out_dim, bias=False)
        self.rm_layer_pe_from_edge = nn.Linear(self.out_dim*3,self.out_dim, bias=False)
        self.curv = [-1., 0.1, 1.]
        self.Ws = []
        self.bias = []
        ...
        for i in range(num_factors):
            if isinstance(dimensions, list):
                ...
                d = dimensions
            pre = torch.randn(d, d).to(device)
            w = pre / (torch.norm(pre, dim=-1, keepdim=True) + EPS)
            self.Ws.append(w)
            self.bias.append(2 * torch.pi * torch.rand(d).to(device))
        self.manifold_list = []
        ...
        for init_curvature  in self.curv:
            ...
            rm_feature = rm_feature.to(device)
            self.rm_feature_list.append(rm_feature)
    ...
    def forward(self, batch, t=None, prefix=None, label=None, **kwargs):
        ...
        h = self.node_emb(batch.x.clone().long()).reshape(num_nodes, -1)
        
        e = self.edge_emb(batch.edge_attr.clone().long()).reshape(num_edges, -1)
    ...
    def encode(self, batch, t=None, prefix=None, label=None, **kwargs):
        return self.forward(batch, t, prefix, label, **kwargs)
    def decode_node_curv(self,x,embeds):
        ...
        x_tmp = self.decode_node(embeds_tensor)
        return x_tmp
    def decode_edge_curv(self,edge_attr,embeds):
        emb_list = []
        for emb in embeds:
            tmp_x = edge_attr
            tmp_x = tmp_x@emb
        ...
        edge_tmp = self.decode_edge(embeds_tensor)
        return edge_tmp
    
    def decode_graph_curv(self,graph_attr,embeds):
        emb_list = []
        for emb in embeds:
            tmp_x = graph_attr
            tmp_x = tmp_x@emb
        ...
        graph_tmp = self.decode_graph(embeds_tensor)
        return graph_tmp

    def decode_node_from_edge_curv(self,graph_attr,embeds):
        emb_list = []
        for emb in embeds:
            tmp_x = graph_attr
            tmp_x = tmp_x@emb
        ...
        graph_tmp = self.decode_node_from_edge(embeds_tensor)
        return graph_tmp

    def decode_edge_from_node_curv(self,graph_attr,embeds):
        emb_list = []
        for emb in embeds:
            tmp_x = graph_attr
            tmp_x = tmp_x@emb
        ...
        graph_tmp = self.decode_edge_from_node(embeds_tensor)
        return graph_tmp

    def decode_nodes_from_edge_curv(self,graph_attr,embeds):
        emb_list = []
        for emb in embeds:
            tmp_x = graph_attr
            tmp_x = tmp_x@emb
        ...
        graph_tmp = self.decode_nodes_from_edge(embeds_tensor)
        return graph_tmp
    ...
    def decode(self, batch, **kwargs):
        ...
        return self.decode_node_curv(batch.x,embeds), self.decode_edge_curv(batch.edge_attr,embeds), self.decode_graph_curv(batch.graph_attr,embeds).flatten()
    ...
    def decode_recon(self, batch, **kwargs):
        ...
        return self.decode_node_from_edge_curv(score_node,embeds), self.decode_edge_from_node_curv(score_edge,embeds), self.decode_nodes_from_edge_curv(batch.edge_attr,embeds), \
               self.decode_pe_from_node_curv(batch.x,embeds), self.decode_pe_from_edge_curv(batch.edge_attr, embeds)
