...
@register_layer("GTE_layer_curv5")
...
@register_network('GraphTransformerEncoder_curv5')
class GraphTransformerEncoder_curv5(nn.Module):
    """
    ...
    def __init__(self, dim_in=0, dim_out=0,  # in_dim, posenc_in_dim, posenc_dim, prefix_dim, hid_dim, out_dim,
        ...
        if self.rm_encode:
            ...
            self.encode_kernel_layer_edge = nn.Linear((len(self.curv1)+1)*self.out_dim,self.out_dim)
            if cfg.encode_layer_norm:
                self.encode_kernel_norm_node = nn.LayerNorm(self.out_dim)
                self.encode_kernel_norm_edge = nn.LayerNorm(self.out_dim)
    ...
    def encode_edge_curv(self,edge_attr):
        ...
        embeds_tensor = self.encode_kernel_layer_edge(embeds_tensor)
        # embeds_tensor = self.encode_kernel_norm_edge(embeds_tensor)
    ...
    def forward(self, batch, t=None, prefix=None, label=None, **kwargs):
        ...
        batch.edge_attr = self.encode_edge_curv(batch.edge_attr)
        if self.final_norm:
...
@register_network('GraphTransformerStructureEncoder_curv5')
class GraphTransformerStructureEncoder_curv5(nn.Module):
    """
