...
if __name__ == '__main__':
    ...
    for run_id, seed, split_index in zip(*run_loop_settings()):
        ...
        if not cfg.optim.seperate:
            optimizer = create_optimizer(model.parameters(),
                                     new_optimizer_config(cfg))
            scheduler = create_scheduler(optimizer, new_scheduler_config(cfg))
        else:
            rm_params = [p for name, p in model.named_parameters() if "rm" in name]
            other_params = [p for name, p in model.named_parameters() if "rm" not in name]
            optimizer = torch.optim.AdamW(other_params, lr=cfg.optim.base_lr, weight_decay=cfg.optim.weight_decay)
            scheduler = create_scheduler(optimizer, new_scheduler_config(cfg))
